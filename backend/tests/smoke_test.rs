...
async fn send_request(
    ...
    let req = req_builder
        .body(if let Some(b) = body {
            simple_server::web::body::Body::from(b)
        } else {
            simple_server::web::body::Body::empty()
        })
        .unwrap();
...
async fn test_chat_completions_requires_auth() {
    ...
    let app = simple_server::web::Router::new().nest("/v1", routes::chat::router(state));
...
async fn test_embeddings_requires_auth() {
    let state = create_test_state().await.unwrap();
    let app = simple_server::web::Router::new().nest("/v1", routes::embeddings::router(state));
...
async fn test_speech_requires_auth() {
    let state = create_test_state().await.unwrap();
    let app = simple_server::web::Router::new().nest("/v1", routes::speech::router(state));
...
async fn test_audio_embeddings_requires_auth() {
    let state = create_test_state().await.unwrap();
    let app = simple_server::web::Router::new().nest("/v1", routes::audio_embeddings::router(state));

    let req = http::Request::builder()
        .method(http::Method::POST)
        .uri("/v1/audio/embeddings")
        .header("Content-Type", "multipart/form-data; boundary=x")
        .body(simple_server::web::body::Body::from("--x--\r\n"))
        .unwrap();
...
async fn gateway_metadata_exposes_only_public_login_configuration() {
    ...
    let response = app.oneshot(http::Request::builder().uri("/.well-known/simple-ai").body(simple_server::web::body::Body::empty()).unwrap()).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let bytes = simple_server::web::body::to_bytes(response.into_body(), 4096).await.unwrap();
